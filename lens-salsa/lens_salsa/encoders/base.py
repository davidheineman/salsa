...
class Encoder(nn.Module, metaclass=abc.ABCMeta):
    ...
    def add_span_tokens(self, begin_of_span: str, end_of_span: str, continuous: bool) -> None:
        ...
        span_tokens = self.tokenizer.get_added_vocab()
        self.begin_of_span_ids += [span_tokens[begin_of_span]]
        self.end_of_span_ids += [span_tokens[end_of_span]]

        # Set up lookup for continuous labels
        self.continuous = continuous
        if continuous:
            self.token_lookup = {v: int(k.replace('<', '').replace('>', '').replace('/','')) for k, v in span_tokens.items()}

    def extract_tag_spans(self, input_ids: List[int], inference: bool = False, filler_token_id: int = None) -> Tuple[List[int], List[int]]:
        ...
        This is changed so that with multiple possible spans, they are assigned 1, 2, 
        ...
        ...
        for i in range(len(input_ids)):
            ...
            for j in range(len(input_ids[i])):
                if input_ids[i][j] in self.begin_of_span_ids:
                    if self.continuous:
                        in_span = self.token_lookup[input_ids[i][j]]
                    else:
                        in_span = self.begin_of_span_ids.index(input_ids[i][j]) + 1

                elif input_ids[i][j] in self.end_of_span_ids:
                    in_span = 0

                # Inference only
                elif inference and input_ids[i][j] == filler_token_id:
                    continue

                elif in_span != 0: # from >
                    input_tokens.append(input_ids[i][j])
                    tag_seq.append(in_span)
            ...
            for i, _ in enumerate(self.begin_of_span_ids):
                if (self.begin_of_span_ids[i] in input_tokens) or (
                    self.end_of_span_ids[i] in input_tokens
                ):
                    raise Exception(
    ...
    def subword_tokenize(self, sample: List[str], inference: bool = False, filler_token_id: int = None) -> Dict[str, torch.Tensor]:
        ...
        input_ids, in_span_mask = self.extract_tag_spans(encoder_input.input_ids, inference, filler_token_id)
        ...
        in_span_mask = self.pad_list(in_span_mask, max_length, self.in_span_mask_pad_id)
    ...
    def concat_sequences(
        ...
        if return_in_span_mask:
            # I believe this is where only the in_span_mask is taken for just the
            # output sentence. Instead, we want both inputs[0] and inputs[1] span
            # masks. Need to be careful about concatenation here.

            concat_input_ids.append(input_ids)

            out_spans, src_spans = inputs[0]["in_span_mask"], inputs[1]["in_span_mask"]
            out_spans = [x.masked_select(x.ne(self.in_span_mask_pad_id)) for x in out_spans.unbind(dim=0)]
            src_spans = [x.masked_select(x.ne(self.in_span_mask_pad_id)) for x in src_spans.unbind(dim=0)]

            in_span_mask = []
            for i in range(len(out_spans)):
                in_span_mask += [self.pad_tensor(torch.cat((out_spans[i], src_spans[i]), dim=0), max_len, self.in_span_mask_pad_id)]
